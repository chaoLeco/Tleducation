...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    if ([text isEqualToString:@"\n"]) {
        ...
        [self sendTextCommentTaped:self.sendButton];
        return NO;
    }
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(textFild: andTextView:)]) {
        return [self.dataSource textFild:text andTextView:textView];
    }
    return YES;
}
@end
